...
satscan.files <- list.files("./01-Data/02-Analytic-Data/SaTScan/output", full=T, pattern = "sqrtprop_ili")
...
for(i in 1:length(clusters.detected)){
  if(is.null(clusters.detected[[i]])){
    ...
    for(ii in 1:length(loc.indices)){
      ...
      # }
      while(!grepl("Overlap|Coordinates", each.cluster[[ii]][2])){
...
save(all.clusters.df, file = "./01-Data/01-Processed-Data/satscan_clusters_ili_spatial.rds")
