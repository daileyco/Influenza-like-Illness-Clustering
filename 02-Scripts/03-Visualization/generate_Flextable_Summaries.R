...
## split by time frame to have manageable table sizes


st.list <- list(summary = table.summary.clusters)
...
caps <- c("Characteristics of Detected Clusters of Influenza-like Illness Rate of Change by Proximity Model / Distance Metric")



## flextable formatting
sft.list <- lapply(1:length(st.list), 
                   function(fti){
                     ...
                     ### delete duplicated grouping variables 
                     #### i don't like the way flextable::as_grouped_data() does it
                     ft[which(duplicated(ft[,1])),1] <- NA
                     ...
                     ft %>%
                       ...
                       #### numbers right
                       align(part = "body", j=3:4, align = "right") %>% 
